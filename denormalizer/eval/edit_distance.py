...
def sentence_error(source, target):
    ...
    return 0 if target.replace(" ", "") == source.replace(" ", "") else 1
...
def preprocess(input_str:str):
    ...
    input_str = re.sub(r"n\s't", "n't", input_str)
    input_str = re.sub(r"n't", " n't", input_str)
    input_str = re.sub(r"%", r" %", input_str)
    input_str = re.sub(r"\$", r" $ ", input_str)
    input_str = re.sub(r"\s+", " ", input_str)
    return input_str

def postprocess(input_str_list:list):
    result_str_list = []
...
def calculate_error_rates(source_file, target_file, config):
    ...
    with open(source_file) as src_file, open(target_file) as tgt_file:
        ...
        for src, tgt in tqdm(zip(src_file, tgt_file)):
            src, tgt = src.strip(), tgt.strip()
            src, tgt = preprocess(src), preprocess(tgt) # insert/remove spaces around ' to standardize different formats.
