...
def denormalize_file(DENORMALIZER, file, beam, outfile):
    ...
    with open(file) as normalized:
        for line in tqdm(normalized):
            denormalized = DENORMALIZER.translate(line.strip().lower(), beam=beam)
            n_sents += 1
...
def main():
    ...
    DENORMALIZER = TransformerModel.from_pretrained(
        ...
        data_name_or_path=f'data-bin/{model}',
        bpe='subword_nmt',
        bpe_codes=f'data-bin/{model}/bpe_code',
