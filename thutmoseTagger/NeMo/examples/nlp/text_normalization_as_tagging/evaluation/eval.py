...
# calculate edit distance on character level.
...
def check(prediction :str, ref :str):
    # the following symbols are kept in both strings
    # _ - / ~ " ' & % $ £ € ¥ #
    # because these are not punctuation marks and they carry semantic meanings
    ...
    #     if char==" " and i!=len(prediction)-1 and not prediction[i+1].isalnum():
    #         continue
    #     temp += char
    # prediction = temp
    # if ref.find("£")!=-1:
    ...
    # if ref.find("μm")!=1:
    #     ref = ref.replace("μm", "um")
    # # deal with equal sign in prediction
    # prediction = prediction.replace("Y=", " ")

    # # deal with ! in prediction
    # temp_idx = prediction.find("!")
    # if temp_idx!=-1:
    #     try:
    #         if prediction[temp_idx-5:temp_idx]!="Yahoo" and prediction[temp_idx-5:temp_idx]!="yahoo":
    #             prediction = prediction[:temp_idx] + prediction[temp_idx+1:]
    #     except:
    ...
    # # deal with , in prediction
    # # remove all , in prediction that are possibly punctuations
    # N = 1
    # temp = find_nth(prediction, ",", N)
    # while temp!=-1:
    #     if temp==0:
    #         prediction=prediction[1:]
    #     try:
    #         if not prediction[temp-1].isnumeric() or not prediction[temp+2].isnumeric():
    #             prediction=prediction[:temp]+prediction[temp+1:]
    #         else:
    #             N += 1
    #     except:
    #         print("find prediction error")
    #         print(prediction)
    #         print(temp)
    #     temp = find_nth(prediction, ',', N)

    # # remove all , that are surrounded by numbers
    # N = 1
    # temp = find_nth(ref, ",", N)
    # while temp!=-1:
    #     if temp==0:
    #         ref=ref[1:]
    #     try:
    #         if ref[temp-1].isnumeric() and ref[temp+1].isnumeric():
    #             ref = ref[:temp] + ref[temp+1:]
    #         else:
    #             N += 1
    #     except:
    #         print("find ref error")
    #         print(ref)
    #         print(temp)
    #     temp = find_nth(ref, ',', N)
    ...
    ref = ref.replace(" ", "")
    return prediction==ref, prediction, ref

def find_extra_character(str1, str2):
    # Find the minimum length of the two strings
    min_length = min(len(str1), len(str2))

    # Iterate through the characters and find the differing one
    for i in range(min_length):
        if str1[i] != str2[i]:
            return str1[i]

    # If no difference is found in the common part, return the extra character
...
if __name__ == "__main__":
    ...
    for i in range(len(inputs)):
        ...
        for ref in references[i]:
            ...
            if "".join(pred_digit_fragments) == "".join(ref_digit_fragments):
                ok_digit = True
            ok, pred, ref = check(predictions[i], ref)
            if ok:
                ok_all = True

        # if references[i][-1].find("\'")!=-1:
        #     print("xxxxxxxxxxxxxx")
        #     print("\tinput=", " ".join(inputs[i]))
        #     print("\ttags=", " ".join(predicted_tags[i]))
        #     print("\tpred_check=", pred)
        #     print("\tref_check=", ref)
        #     print("\tpred=", predictions[i])
        #     print("\tref=", references[i][-1])
        ...
        try: 
            if not ok_digit:
                ...
                # elif references[i][-1].find("-")!=-1 or predictions[i].find("-")!=-1:
                #     hyphen_errors += 1
                #     print("hyphen errors: ")
                #     print("\tinput=", " ".join(inputs[i]))
                #     print("\ttags=", " ".join(predicted_tags[i]))
                ...
                if edit_distance(pred, ref)<=1:
                    ...
                    if len(pred) == len(ref):
                        ...
                        char = find_extra_character(pred, ref)
                        if char in tolerable:
                            soft_errors_sym += 1
    ...
    # print("hyphen errors: ", hyphen_errors)
    ...
    print("\tsoft errors: ", soft_errors)
    print("\t\tsoft error related to subsitution: ", soft_errors_sub)
    print("\t\tsoft error related to symbols: ", soft_errors_sym)